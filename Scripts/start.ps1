...
Set-ItemProperty -path "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce" -name "myScript" -value ('start powershell -ExecutionPolicy bypass -f "' + (Join-Path (Get-ScriptPath) "newMachineConfig.ps1" ) + '"')
